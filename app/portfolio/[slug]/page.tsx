...
import { SEO_PORTFOLIO } from "@/lib/seo-config";
...
export async function generateStaticParams() {
  return SEO_PORTFOLIO.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const project = SEO_PORTFOLIO.find((s) => s.slug === slug);
  if (!project) return {};

  const title = `${project.title} | Portfolio Case Study`;
  const description = `${project.title} case study by Menarc Solutions. ${project.desc}. Discover how we engineered high-performance digital architecture and automated business systems.`;

  return {
    title,
    description,
    alternates: {
      canonical: `https://menarc.in/portfolio/${slug}`,
    },
    openGraph: {
      title,
      description,
      url: `https://menarc.in/portfolio/${slug}`,
    },
  };
}
    ...
    <div className="pt-24 pb-16 bg-white text-[#1d1d1f] select-none relative overflow-hidden">
      ...
      <section className="container-custom py-16 md:py-24 relative z-20">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#f5f5f7] border border-[#d2d2d7]/60 mb-6 shadow-md">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868b]">Featured Case Study</span>
          </div>

          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter mb-8 leading-[1.08]">
            <span className="block text-[#1d1d1f] mb-2">Architectural Excellence</span>
          ...
          <p className="text-lg md:text-xl text-[#86868b] leading-relaxed mb-12 max-w-3xl mx-auto tracking-tight font-normal">
            {project.desc} We delivered uncompromising engineering quality, combining cinematic UI/UX aesthetics with ultra-fast Next.js performance and automated n8n workflows.
          </p>
      ...
      <section className="container-custom py-20 border-b border-[#d2d2d7]/60 relative z-20">
        <div className="max-w-3xl mx-auto text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold mb-4 tracking-tighter text-[#1d1d1f]">Project Highlights & Impact</h2>
          <p className="text-[#86868b] text-sm tracking-tight">We reject superficial vanity metrics. Every platform is architected to deliver measurable business growth and operational efficiency.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {[
            ...
            },
            {
              ...
              desc: "Transparent milestone-based delivery with zero hidden fees. The client knew exactly what they were investing in from day one.",
            },
          ].map((val, idx) => (
