...
import { SEO_RESOURCES } from "@/lib/seo-config";
...
export async function generateStaticParams() {
  return SEO_RESOURCES.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const resource = SEO_RESOURCES.find((s) => s.slug === slug);
  if (!resource) return {};

  const title = `${resource.title} | Free Architectural Blueprint`;
  const description = `${resource.desc} Download the complete open-source enterprise blueprint and starter kit by Menarc Solutions.`;

  return {
    title,
    description,
    alternates: {
      canonical: `https://menarc.in/resources/${slug}`,
    },
    openGraph: {
      title,
      description,
      url: `https://menarc.in/resources/${slug}`,
    },
  };
}
    ...
    <div className="pt-24 pb-16 bg-white text-[#1d1d1f] select-none relative overflow-hidden">
      ...
      <section className="container-custom py-16 md:py-24 relative z-20">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#f5f5f7] border border-[#d2d2d7]/60 mb-6 shadow-md">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868b]">Enterprise Starter Kit</span>
          </div>

          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter mb-8 leading-[1.08]">
            <span className="block text-[#1d1d1f] mb-2">Open-Source Blueprint</span>
          ...
          <p className="text-lg md:text-xl text-[#86868b] leading-relaxed mb-12 max-w-3xl mx-auto tracking-tight font-normal">
            {resource.desc} We deliver uncompromising engineering blueprints, combining silicon valley best practices with ultra-fast Next.js tracking performance and automated n8n workflows.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link
              href={`/contact?resource=${slug}`}
            ...
            >
              Download Complete Blueprint <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href="/resources"
      ...
      <section className="container-custom py-20 border-b border-[#d2d2d7]/60 relative z-20">
        <div className="max-w-3xl mx-auto text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold mb-4 tracking-tighter text-[#1d1d1f]">What Inside This Starter Kit?</h2>
          <p className="text-[#86868b] text-sm tracking-tight">We reject superficial PDF lead magnets. Every resource is a fully functional architectural blueprint engineered for immediate deployment.</p>
        </div>
