...
import { SEO_SEO_SERVICES } from "@/lib/seo-config";
...
export async function generateStaticParams() {
  return SEO_SEO_SERVICES.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const service = SEO_SEO_SERVICES.find((s) => s.slug === slug);
  if (!service) return {};

  const title = `${service.title} Services | Menarc Solutions`;
  const description = `Premium ${service.title.toLowerCase()} by Menarc Solutions. We engineer native JSON-LD structured data, 100/100 Core Web Vitals, and Generative Engine Optimization (GEO).`;

  return {
    title,
    description,
    alternates: {
      canonical: `https://menarc.in/seo-services/${slug}`,
    },
    openGraph: {
      title,
      description,
      url: `https://menarc.in/seo-services/${slug}`,
    },
  };
}
    ...
    <div className="pt-24 pb-16 bg-white text-[#1d1d1f] select-none relative overflow-hidden">
      ...
      <section className="container-custom py-16 md:py-24 relative z-20">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#f5f5f7] border border-[#d2d2d7]/60 mb-6 shadow-md">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868b]">Advanced Search Architecture</span>
          </div>

          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter mb-8 leading-[1.08]">
            <span className="block text-[#1d1d1f] mb-2">Technical Discoverability</span>
          ...
          <p className="text-lg md:text-xl text-[#86868b] leading-relaxed mb-12 max-w-3xl mx-auto tracking-tight font-normal">
            We deliver uncompromising technical excellence for your {service.title.toLowerCase()} requirements. Combining native structured data with machine-readable LLM ingestion endpoints.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            >
              Request SEO Audit & Strategy <ArrowRight className="w-4 h-4" />
            </Link>
            <Link
              href="/pricing"
      ...
      <section className="container-custom py-20 border-b border-[#d2d2d7]/60 relative z-20">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-[#86868b] text-sm tracking-tight">We reject superficial keyword stuffing. Every platform is architected for deep semantic indexing and AI crawler dominance.</p>
        </div>
          ...
          {[
            {
              ...
              title: "100/100 Core Web Vitals Guarantee",
              desc: "We optimize server hydration, font subsetting, and image compression to ensure lightning-fast First Contentful Paint (FCP) and zero CLS.",
            },
            {
              icon: <Shield className="w-8 h-8 text-[#1d1d1f]" />,
              title: "Native JSON-LD Schema Markup",
              desc: "We inject robust Organization, LocalBusiness, and Breadcrumb schemas directly into server-rendered script tags for search engine bots.",
            },
            {
              icon: <CheckCircle2 className="w-8 h-8 text-[#1d1d1f]" />,
              title: "Generative Engine Optimization (GEO)",
              desc: "Dedicated `llms.txt` and `robots.ts` directives ensure your brand is accurately summarized by ChatGPT, Claude, and Perplexity.",
            },
            {
              icon: <Award className="w-8 h-8 text-[#1d1d1f]" />,
              title: "Transparent Performance Tracking",
              desc: "Complete Google Search Console integration and automated sitemap generation to monitor your organic indexing growth.",
            },
          ].map((val, idx) => (
