...
// SVGs with forced brand colors for light theme
const logos = {
  Nextjs: (
    <svg viewBox="0 0 180 180" className="w-8 h-8 fill-[#1d1d1f]">
  ...
  Gemini: (
    <svg viewBox="0 0 24 24" className="w-8 h-8 fill-[#4285F4]">
      <path d="M23.63 10.976c-3.111-1.077-5.59-3.556-6.666-6.667-.53-1.53-.53-1.53-1.061 0-1.077 3.11-3.555 5.59-6.666 6.666-1.53.53-1.53.53 0 1.06 3.11 1.077 5.59 3.556 6.667 6.667.53 1.53.53 1.53 1.06 0 1.077-3.111 3.556-5.59 6.666-6.667.53-.53.53-.53 0-1.06zM8.344 19.06c-1.63-1.636-.566-2.94-1.87-3.506-3.506-.278-.804-.278-.804-.557 0-.566 1.636-1.87 2.94-3.506 3.506-.804.278-.804.278 0 .557 1.636.566 2.94 1.87 3.506 3.506.278.804.278.804.557 0 .566-1.636 1.87-2.94 3.506-3.506.804-.278.804-.278 0-.557z" />
    </svg>
  ),
  Stripe: (
...
export default function TechEcosystem() {
  ...
  return (
    <section
      id="tech-ecosystem"
      className="bg-[#f5f5f7] relative overflow-hidden py-16 md:py-24 flex flex-col md:flex-row items-center justify-center"
    >
      {/* Elegant Top SVG Curve Transition from Services */}
      ...
      {/* Subtle Apple ambient blueprint grid */}
      ...
      <div className="container-custom relative z-10 flex flex-col md:flex-row items-center h-full w-full gap-8 md:gap-12">
        ...
        <div className="w-full md:w-1/2 z-10 text-center md:text-left mb-12 md:mb-0 md:-mt-10">
          <AnimatedSection>
        ...
        <div className="relative w-full md:w-1/2 h-[26rem] md:h-[36rem] flex items-center justify-center overflow-visible">
          <div className="relative w-[14rem] h-[14rem] md:w-[32rem] md:h-[32rem] flex items-center justify-center">
            
            {/* Center Circle with "M" */}
            <div className="w-16 h-16 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-[#f5f5f7] to-[#ffffff] border-2 border-menarc-gold shadow-lg flex items-center justify-center relative z-20">
              <span className="font-display font-black text-3xl md:text-5xl text-transparent bg-clip-text bg-gradient-to-br from-[#D4AF37] via-[#c5a059] to-[#D4AF37]">
                M
              </span>
            </div>
            ...
            {[...Array(orbitCount)].map((_, orbitIdx) => {
              ...
              return (
                <div
                  key={orbitIdx}
                  className="absolute rounded-full border border-dashed border-[#d2d2d7]"
                  style={{
                    width: size,
                    height: size,
                    animation: `orbitSpin ${15 + orbitIdx * 8}s linear infinite`,
                  }}
                >
                  {iconConfigs
                    ...
                    .map((cfg, iconIdx) => {
                      ...
                      return (
                        <div
                          key={iconIdx}
                          className="absolute bg-white border border-[#d2d2d7]/60 rounded-full p-2 md:p-3 shadow-sm flex items-center justify-center transition-all duration-300 hover:scale-110 hover:border-menarc-gold hover:shadow-md group"
                          style={{
                            left: `${x}%`,
                            top: `${y}%`,
                            transform: "translate(-50%, -50%)",
                            // Keep icons upright by spinning backwards
                            animation: `orbitSpinReverse ${15 + orbitIdx * 8}s linear infinite`,
                          }}
                        >
